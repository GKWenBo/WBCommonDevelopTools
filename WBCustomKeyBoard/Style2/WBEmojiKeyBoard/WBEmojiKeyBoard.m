...
#define kWB_EmojiKeyboardFrame CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds), 262)
#define  kWB_EmojiToolBarHeight 30

#pragma mark --------  Botttom ToolBar  --------
#pragma mark
@interface WB_EmojiToolBar : UIView
@property (assign, nonatomic) WB_EmojiType showType;
+ (instancetype)toolBarWithEmojis:(NSArray *)emojis;
@property (strong, nonatomic) void (^actionHandler)(WB_EmojiType);
@property (strong, nonatomic) void (^deleteHandler)();

@end

@implementation WB_EmojiToolBar
{
    BOOL _beginDelete;
    CGFloat _time;
    NSArray *_emojis;
...
+ (instancetype)toolBarWithEmojis:(NSArray *)emojis{
    WB_EmojiToolBar *toolbar = [self new];
...
- (void)loadEmojis:(NSArray *)emojis{
    _emojis = emojis;
    [self loadUI];
    self.showType = WB_EmojiTypePeople;
}
...
- (void)setShowType:(WB_EmojiType)showType{
    _showType = showType;
...
@interface WBEmojiKeyBoard () <WBEmojiCollectionViewDelegate>

@property (strong, nonatomic) WBEmojiCollectionView *emojiPageView;
@property (strong, nonatomic) WB_EmojiToolBar *toolBar;

@property (strong, nonatomic) NSArray *emojiData;

@end

@implementation WBEmojiKeyBoard

+ (instancetype)keyboard{
    static dispatch_once_t onceToken;
    static WBEmojiKeyBoard *_sharedKeyboard;
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:kWB_EmojiKeyboardFrame]){
        self.clipsToBounds = NO;
        _emojiData = [WBEmoji allEmojis];
        [self loadUI];
...
- (void)loadUI{
    _emojiPageView = [[WBEmojiCollectionView alloc] initWithFrame:self.bounds];
    _emojiPageView.emojiDelegate = self;
    _toolBar = [WB_EmojiToolBar toolBarWithEmojis:_emojiData];
    __weak typeof(self) ws = self;
    __weak typeof(_emojiPageView) wPageView = _emojiPageView;
    __weak typeof(_toolBar) wToolBar = _toolBar;
    [_toolBar setActionHandler:^(WB_EmojiType showType) {
...
#pragma mark --------  WB_EmojiCollectionViewDelegate  --------
#pragma mark
