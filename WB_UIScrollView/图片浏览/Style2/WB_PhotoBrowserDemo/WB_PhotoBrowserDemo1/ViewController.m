...
- (void)tapImage:(UITapGestureRecognizer *)tap
{
    ...
    for (int i = 0; i<count; i++) {
        ...
        photo.srcImageView = [self.view viewWithTag:i + kBaseTag]; // 来源于哪个UIImageView
        [photos addObject:photo];
    }
    
    // 2.显示相册
    MJPhotoBrowser *browser = [[MJPhotoBrowser alloc] init];
    browser.currentPhotoIndex = tap.view.tag - kBaseTag; // 弹出相册时显示的第一张图片是？
