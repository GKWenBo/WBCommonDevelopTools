...
- (void)setupUI {
    ...
    [self.normalImageArray enumerateObjectsUsingBlock:^(NSString *  normalImageName, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        button.titleLabel.font = kWB_PFR_FONT10;
        ...
        [button setTitle:self.titleArray[idx] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        [button setTitleColor:[UIColor wb_appMainRedColor] forState:UIControlStateSelected];
        ...
        if (idx == 2) {
            ...
            [button wb_verticalCenterImageAndTitle];
        }
...
- (NSArray *)normalImageArray {
    if (!_normalImageArray) {
        _normalImageArray = @[@"tab_home_normal",
                              @"tab_team_normal",
                              @"tab_center",
                              @"tab_message_normal",
                              @"tab_mine_normal"];
...
- (NSArray *)selectedImageArray {
    if (!_selectedImageArray) {
        _selectedImageArray = @[@"tab_home_selected",
                                @"tab_team_selected",
                                @"tab_center",
